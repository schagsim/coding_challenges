namespace advent_of_code.aoc_2024.aoc_2024_02;
...
public static class AoC202402
{
    ...
    private static string[] ReadInputFile()
    {
        ...
        var inputFilePath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "aoc-2024", "aoc-2024-02", "input.txt");
        return File.ReadAllLines(inputFilePath);
    }

    private static List<string> SplitAndTrimInput(string inputString, char delimiter)
    {
        return inputString.Split(delimiter).Select(s => s.Trim()).ToList();

    }

    private static bool IsGamePossible(string gameRaw, out MaxCubes maxCubes)
    {
        maxCubes.maxRed = 0;
        maxCubes.maxGreen = 0;
        maxCubes.maxBlue = 0;
        ...
        {
            var drawData = SplitAndTrimInput(specificDraw, ' ');
            var dataValue = int.Parse(drawData[0]);
            switch (drawData[1])
            {
                case "red":
    ...
    /// <summary>
    /// Immediately check whether the game is playable
    /// </summary>
    /// <param name="line"></param>
    /// <param name="maxCubes"></param>
    /// <returns></returns>
    private static bool ParseGame(string line, out MaxCubes maxCubes)
    {
        maxCubes.maxRed = 0;
        maxCubes.maxGreen = 0;
        maxCubes.maxBlue = 0;
        ...
        foreach (var specificGameRaw in specificGamesRaw)
        {
            var currentMax = new MaxCubes();
            var gamePossible = IsGamePossible(specificGameRaw, out currentMax);
            if (currentMax.maxRed > maxCubes.maxRed) maxCubes.maxRed = currentMax.maxRed;
    ...
    public static void ExecuteFirst()
    {
        ...
        //var isFirstGamePossible = ParseGame(input[5]);
        //Console.WriteLine($"First game possible: {isFirstGamePossible}");
        ...
        Console.WriteLine($"Sum of minimal possible games: {sumMax}");
    }
